...
#
# bash <(curl -s https://raw.githubusercontent.com/crbowman/dotfiles/master/bootstrap.sh)
...
if [[ ! -d ~/.dotfiles ]]; then
  ...
  if [[ $USER == curtis ]]; then
    dfrepo=git@github.com:crbowman/dotfiles.git
  else
    dfrepo=https://github.com/crbowman/dotfiles
  fi
